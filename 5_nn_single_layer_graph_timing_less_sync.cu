...
#define SYNC_INTERVAL 25 // Only synchronize every X epochs
...
int main() {
    ...
    for (int epoch = 0; epoch < EPOCHS; ++epoch) {
        ...
        if ((epoch + 1) % SYNC_INTERVAL == 0 && epoch % 10 == 0) {
            ...
            std::cout << "Epoch time (avg over last " << SYNC_INTERVAL << " epochs): " << epoch_time_ms << " ms" << std::endl;
        }
    }
    ...
    for (int epoch = 0; epoch < EPOCHS; ++epoch) {
        ...
        // Only synchronize every SYNC_INTERVAL epochs or on the last epoch
        if ((epoch + 1) % SYNC_INTERVAL == 0 || epoch == EPOCHS - 1) {
            cudaStreamSynchronize(baseline_stream);
        }
        ...
        if ((epoch + 1) % SYNC_INTERVAL == 0 && epoch % 10 == 0) {
            std::cout << "Baseline epoch " << epoch + 1 << " time (avg over last " << SYNC_INTERVAL << " epochs): " << epoch_time_ms << " ms" << std::endl;
        }
    }
